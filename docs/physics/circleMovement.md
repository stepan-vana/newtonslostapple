...
**Frequnecy** is how many revolutions the body does in 1 second. The symbol is $f$ and the SI unit is $Hz$ (hertz).  
**Period** is how long it takes for a body to make one full revolution. The symbol is $T$ and the SI unit is $s$ (second).  
The convert equation for frequency and peroid is:
...
**Angular velocity** is like a speed, but instead of distance we use angles, the symbol is $\omega$ and the SI unit is $\mathrm{rad\cdot s^{-1}}$ (radians per second), where $ 2\pi \space rad= 360\space °$. You can calculate the angular momentum like this:
